...
def save_checkpoint(best_auc, model, optimizer, epoch):
    ...
    torch.save({
        ...
        'best_auc': best_auc,
    }, os.path.join('/media/data3/users/loctx/MRM/Fold4/', config.model_name + '_' + config.experiment + '_checkpoint_best.pth'))
...
def train_epoch(model, optimizer, criterion, criterion_Cross, scheduler_teacher_model, train_dataloader):
    ...
    for inputs1, target in train_dataloader:
        ...
        #print(f"Input shape to model: {inputs1.shape}")
...
def train(i, config=config):
    ...
    # datasets
    train_dataloader, val_dataloader, test_dataloader, num_classes = load_data(32)
    ...
    for epoch in range(1, config.max_epoch + 1):
        ...
        # if epoch == config.max_epoch:
        #     path = os.path.join('/media/data3/users/loctx/MRM/Fold4', config.model_name + '_' + config.experiment + '_checkpoint_best.pth')
        #     load_model = load_checkpoint(path, model, optimizer)
        #     test_loss, test_auc, test_TPR = test_epoch(load_model, criterion, criterion_Cross, test_dataloader)
