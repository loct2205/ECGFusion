...
def cross_validation_splits(data, labels, fold_csv_path='F:/dataset/2018/REFERENCE_5fold.csv', fold_for_test=2):
    """
    Chia train/val/test theo REFERENCE_5fold.csv
    """
    import pandas as pd

    df_folds = pd.read_csv(fold_csv_path)
    recording_to_fold = dict(zip(df_folds['Recording'], df_folds['fold']))

    file_ids = df_folds['Recording'].tolist()

    assert len(file_ids) == len(data), "Số lượng file và data không khớp"

    test_mask = np.array([recording_to_fold[rid] == fold_for_test for rid in file_ids])
    train_val_mask = ~test_mask

    x_test = data[test_mask]
    y_test = labels[test_mask]
    x_train_val = data[train_val_mask]
...
def load_data(batch_size):
    num_classes = 9
    data_dir = "/media/data3/users/loctx/MRM/Fold0"
    ...
    return train_loader, val_loader, test_loader, num_classes
# def load_data(batch_size, fold=0):
#     """
#     Load train/val/test datasets theo fold.
#     """
#     num_classes = 9
#     data_dir = f"/media/data3/users/loctx/MRM/Fold{fold}"
#
#     # Load numpy arrays
#     x_train = np.load(os.path.join(data_dir, "x_train.npy"))
#     y_train = np.load(os.path.join(data_dir, "y_train.npy"))
...
#     y_test = np.load(os.path.join(data_dir, "y_test.npy"))
#
#     # Preprocess signals (e.g., normalization, scaling, etc.)
#     x_train, x_val, x_test = preprocess_signals(x_train, x_val, x_test)
#
