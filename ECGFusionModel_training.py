...
class ECGFusionDataset(Dataset):
    def __init__(self, file_ids, label_path, resnet_dir, swin_dir, csv_file="./KFold/AttentionModel/Fold4/log.csv", num_classes = 9):
...
def get_dataloaders(train_files, test_files, label_path,
                    resnet_dir="/media/data3/users/minhnb/Fold4/feature", swin_dir="./KFold/Feature/Fold4",
                    batch_size=1, num_workers=4):
    train_dataset = ECGFusionDataset(train_files, label_path, resnet_dir, swin_dir, csv_file="./KFold/AttentionModel/Fold4/log_train.csv")
    test_dataset = ECGFusionDataset(test_files, label_path, resnet_dir, swin_dir, csv_file="./KFold/AttentionModel/Fold4/log_test.csv")
...
class ECGFusionModel(nn.Module):
    ...
    def forward(self, resnet_feat, swin_feat):
        """
        resnet_feat: (segment, 512)
        swin_feat: (segment, seq_len, 768)
        """
        # add batch dimension
        R = resnet_feat.unsqueeze(0)  # (1, Lr, 512)
        S = swin_feat.flatten(0, 1).unsqueeze(0)  # (1, Ls, 768)

        # Projector same dim
        # R_proj = self.resnet_proj(R)  # (1, Lr, 768)
        # S_proj = self.swin_proj(S)    # (1, Ls, 768)
        R_proj = self.resnet_proj(R)  # (1, Lr, 256)
        S_proj = self.swin_proj(S)  # (1, Ls, 256)

        # Cross attention
        R_to_S, _ = self.cross_attn_R_to_S(query=R_proj, key=S_proj, value=S_proj)
        S_to_R, _ = self.cross_attn_S_to_R(query=S_proj, key=R_proj, value=R_proj)

        # Mean pooling and fusion
        pooled_R = R_to_S.mean(dim=1)  # (1, 256)
        pooled_S = S_to_R.mean(dim=1)  # (1, 256)

        fusion = torch.cat([pooled_R, pooled_S], dim=-1)  # (1, 512)
        fusion = self.fusion_proj(fusion)  # (1, 1024)
        logits = self.classifier(fusion)  # (1, num_classes)
        return logits


def train_fusion_model(model, train_loader, val_loader, device, num_epochs=30, lr=1e-4,
                       weight_decay=1e-4, early_stop_patience=5, save_path="./KFold/AttentionModel/Fold4/best_fusion_model.pth",
                       log_txt_path="./KFold/AttentionModel/Fold4/fusion_training_log.txt"):
    model.to(device)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for resnet_feat, swin_feat, labels, _ in tqdm(train_loader, desc=f"Epoch {epoch}/{num_epochs}"):
                ...
                for i in range(batch_size):
                    logits = model(resnet_feat[i], swin_feat[i])
...
def evaluate_voting(model, test_loader, device, model_name, threshold=0.5):
    ...
    with torch.no_grad():
        for resnet_feat, swin_feat, labels, file_id_batch in test_loader:
            ...
            logits_batch = []
            for i in range(batch_size):
                logits = model(resnet_feat[i], swin_feat[i])  # (1, num_classes)
                logits_batch.append(logits)
    ...
    df.to_csv(f"./KFold/AttentionModel/Fold4/{model_name}_voting_results.csv", index=False)
    ...
    with open(f"./KFold/AttentionModel/Fold4/{model_name}_voting_results.txt", "w") as f:
        f.write("Mean Voting:\n")
    ...
    plot_confusion_matrix(y_true_single, y_mean_single,
                          ...
                          f"./KFold/AttentionModel/Fold4/{model_name}_mean_confusion_matrix.png")

    plot_confusion_matrix(y_true_single, y_median_single,
                          f"{model_name} - Median Voting - Confusion Matrix",
                          f"./KFold/AttentionModel/Fold4/{model_name}_median_confusion_matrix.png")
    plot_confusion_matrix(
        y_true_mean_o, y_pred_mean_o,
        title=f"{model_name} - Mean Voting - Confusion Matrix",
        filename=f"./KFold/AttentionModel/Fold4/confusion_matrix/{model_name}_mean_confusion_matrix.png"
    )

    plot_confusion_matrix(
        y_true_median_o, y_pred_median_o,
        title=f"{model_name} - Median Voting - Confusion Matrix",
        filename=f"./KFold/AttentionModel/Fold4/confusion_matrix/{model_name}_median_confusion_matrix.png"
    )
    print("Completed!")
...
state_dict = torch.load("./KFold/AttentionModel/Fold4/best_fusion_model.pth", map_location=device)
model.load_state_dict(state_dict)
evaluate_voting(model, test_loader, device, model_name = "best_fusion_model_fold4")
