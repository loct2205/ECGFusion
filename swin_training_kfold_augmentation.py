...
class ECGDataset(Dataset):
    def __init__(self, mat_dir, label_path, file_list, csv_file="./KFold/Model/Fold4/log.csv", window_size=6 * 500,
...
def train_model(train_loader, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        for model, optimizer, scheduler, model_name in zip(
            ...
            scheduler.step()
            # torch.save(model.state_dict(), f"./KFold/Fold4/{epoch}_{model_name}.pth")
            torch.save({
                'epoch': epoch,
                ...
                'loss': epoch_loss,
                'acc': epoch_acc,
            }, f"./KFold/Model/Fold4/swin_checkpoint_epoch{epoch}.pth")
...
def evaluate_model(model, test_loader, model_name):
    ...
    all_targets = []
    with torch.no_grad():
        for x_batch, y_batch, _ in tqdm(test_loader, desc=f"Evaluating {model_name}"):
...
def evaluate_voting(model, test_loader, device, model_name, threshold=0.5):
    ...
    df.to_csv(f"./KFold/Model/Fold4/{model_name}_voting_results.csv", index=False)
    ...
    with open(f"./KFold/Model/Fold4/{model_name}_voting_results.txt", "w") as f:
        f.write("Mean Voting:\n")
...
train_loader, test_loader = get_dataloaders(mat_dir, label_path, train_files, test_files)
...
################
#Test
checkpoint_dir = './KFold/Model/Fold4'
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
# def save_results_to_file(epoch, epoch_loss, epoch_acc, file_path="./KFold/Model/Fold4/result_test_epoch.txt"):
#     with open(file_path, 'a') as f:
#         f.write(f"Epoch {epoch}: Loss = {epoch_loss:.4f}, Accuracy = {epoch_acc:.4f}\n")
#
#
# def save_results_to_file2(epoch, epoch_loss, epoch_acc, file_path="./KFold/Model/Fold4/result_train_epoch.txt"):
#     with open(file_path, 'a') as f:
#         f.write(f"Epoch {epoch}: Loss = {epoch_loss:.4f}, Accuracy = {epoch_acc:.4f}\n")
