...
class ECGDataset(Dataset):
    def __init__(self, mat_dir, label_path, file_list, csv_file="./Resnet_attention/log.csv",
...
def train_model(model, train_loader, val_loader, num_classes, device,
                save_path="./Resnet_attention/resnet1d_attention.pth",
                num_epochs=14, learning_rate=0.001, weight_decay=5e-4,
...
def evaluate_voting(model, test_loader, device, model_name, threshold=0.5):
    ...
    df.to_csv(f"./Resnet_attention/{model_name}_voting_results.csv", index=False)
    ...
    with open(f"./Resnet_attention/{model_name}_voting_results.txt", "w") as f:
        f.write("Mean Voting:\n")
...
#model.load_state_dict(torch.load(f"./Resnet_attention/resnet1d_attention.pth", map_location=device))
